...
addon.data.target.default_settings = {
	enabled = true,
	width = 300,
	height = 12,
	fontsize = 10,
...
addon.data.target.class = 'WARRIOR'
addon.data.target.guid = 0

addon.data.target.main_swing_timer = 0.00001
addon.data.target.prev_main_weapon_speed = 2
addon.data.target.main_weapon_speed = 2
addon.data.target.main_weapon_id = GetInventoryItemID("target", 16)
addon.data.target.main_speed_changed = false

addon.data.target.off_swing_timer = 0.00001
addon.data.target.prev_off_weapon_speed = 2
addon.data.target.off_weapon_speed = 2
addon.data.target.off_weapon_id = GetInventoryItemID("target", 17)
addon.data.target.has_offhand = false
addon.data.target.off_speed_changed = false

addon.data.target.LoadSettings = function()
    -- If the carried over settings dont exist then make them
    if not character_target_settings then
        character_target_settings = {}
    end
    -- If the carried over settings aren't set then set them to the defaults
    for setting, value in pairs(addon.data.target.default_settings) do
...
addon.data.target.RestoreDefaults = function()
    for setting, value in pairs(addon.data.target.default_settings) do
        character_target_settings[setting] = value
    end
    addon.data.target.UpdateVisualsOnSettingsChange()
    addon.data.target.UpdateConfigPanelValues()
end

--[[============================================================================================]]--
--[[====================================== LOGIC RELATED =======================================]]--
--[[============================================================================================]]--
addon.data.target.OnPlayerTargetChanged = function()
    if UnitExists("target") then
        addon.data.target.class = UnitClass("target")[2]
        addon.data.target.guid = UnitGUID("target")
        addon.data.target.ZeroizeSwingTimers()
    end
end

addon.data.target.OnInventoryChange = function()
    ...
    if addon.data.target.main_weapon_id ~= new_main_guid then
        addon.data.target.UpdateMainWeaponSpeed()
        addon.data.target.ResetMainSwingTimer()
    end
    addon.data.target.main_weapon_id = new_main_guid
    -- Check for an off hand weapon change
    if addon.data.target.off_weapon_id ~= new_off_guid then
        addon.data.target.UpdateOffWeaponSpeed()
        addon.data.target.ResetOffSwingTimer()
    end
    addon.data.target.off_weapon_id = new_off_guid
end

addon.data.target.OnUpdate = function(elapsed)
    if character_target_settings.enabled and UnitExists("target") then
        ...
        if addon.data.target.main_weapon_speed == 0 then
            addon.data.target.main_weapon_speed = 2
        end
        if addon.data.target.off_weapon_speed == 0 then
            addon.data.target.off_weapon_speed = 2
        end
        -- If the weapon speed changed for either hand then a buff occured and we need to modify the timers
        if addon.data.target.main_speed_changed or addon.data.target.off_speed_changed then
            local main_multiplier = addon.data.target.main_weapon_speed / addon.data.target.prev_main_weapon_speed
            addon.data.target.main_swing_timer = addon.data.target.main_swing_timer * main_multiplier
            if addon.data.target.has_offhand then
                local off_multiplier = (addon.data.target.off_weapon_speed / addon.data.target.prev_off_weapon_speed)
                addon.data.target.off_swing_timer = addon.data.target.off_swing_timer * off_multiplier
            end
        end
        -- Update the main hand swing timer
        addon.data.target.UpdateMainSwingTimer(elapsed)
        -- Update the off hand swing timer
        addon.data.target.UpdateOffSwingTimer(elapsed)
    end
    -- Update the visuals
        addon.data.target.UpdateVisualsOnUpdate()
end

addon.data.target.OnCombatLogUnfiltered = function(combat_info)
    local _, event, _, source_guid, _, _, _, dest_guid, _, _, _, _, spell_name, _ = unpack(combat_info)
    if (source_guid == addon.data.target.guid) then
        ...
        elseif (event == "SWING_MISSED") then
            ...
            if (dest_guid == addon.data.player.guid) then
                is_player = true
            end
            local miss_type, is_offhand = select(12, unpack(combat_info))
            addon.data.core.MissHandler("target", miss_type, is_offhand, is_player)
        elseif (event == "SPELL_DAMAGE") or (event == "SPELL_MISSED") then
            local _, _, _, _, _, _, spell_id = GetSpellInfo(spell_name)
            addon.data.core.SpellHandler("target", spell_id)
        end
    end
    
end

addon.data.target.ResetMainSwingTimer = function()
    if UnitExists("target") then
        addon.data.target.main_swing_timer = addon.data.target.main_weapon_speed
    end
end

addon.data.target.ResetOffSwingTimer = function()
    if addon.data.target.has_offhand and UnitExists("target") then
        addon.data.target.off_swing_timer = addon.data.target.off_weapon_speed
    end
end

addon.data.target.ZeroizeSwingTimers = function()
    addon.data.target.main_swing_timer = 0.0001
    addon.data.target.off_swing_timer = 0.0001
end

addon.data.target.UpdateMainSwingTimer = function(elapsed)
    if character_target_settings.enabled and UnitExists("target") then
        if addon.data.target.main_swing_timer > 0 then
            addon.data.target.main_swing_timer = addon.data.target.main_swing_timer - elapsed
            if addon.data.target.main_swing_timer < 0 then
                addon.data.target.main_swing_timer = 0
            end
        end
    end
end

addon.data.target.UpdateOffSwingTimer = function(elapsed)
    if character_target_settings.enabled and UnitExists("target") then
        if addon.data.target.has_offhand then
            if addon.data.target.off_swing_timer > 0 then
                addon.data.target.off_swing_timer = addon.data.target.off_swing_timer - elapsed
                if addon.data.target.off_swing_timer < 0 then
                    addon.data.target.off_swing_timer = 0
                end
            end
        end
    end
...
addon.data.target.UpdateMainWeaponSpeed = function()
    if UnitExists("target") then
        -- Handle the nil when first selecting target
        if addon.data.target.main_weapon_speed then
            addon.data.target.prev_main_weapon_speed = addon.data.target.main_weapon_speed
        else
            addon.data.target.prev_main_weapon_speed, _ = UnitAttackSpeed("target")
        end
        -- Update the weapon speed
        addon.data.target.main_weapon_speed, _ = UnitAttackSpeed("target")
        if addon.data.target.main_weapon_speed ~= addon.data.target.prev_main_weapon_speed then
            addon.data.target.main_speed_changed = true
        else
            addon.data.target.main_speed_changed = false
        end
    end
...
addon.data.target.UpdateOffWeaponSpeed = function()
    if UnitExists("target") then
        -- Handle the nil when first selecting target
        if addon.data.target.off_weapon_speed then
            addon.data.target.prev_off_weapon_speed = addon.data.target.off_weapon_speed
        else
            _, addon.data.target.prev_off_weapon_speed = UnitAttackSpeed("target")
        end
        -- Update the weapon speed
        _, addon.data.target.off_weapon_speed = UnitAttackSpeed("target")
        -- Check to see if we have an off-hand
        if (not addon.data.target.off_weapon_speed) or (addon.data.target.off_weapon_speed == 0) then
            addon.data.target.has_offhand = false
        else
            addon.data.target.has_offhand = true
        end
        if addon.data.target.off_weapon_speed ~= addon.data.target.prev_off_weapon_speed then
            addon.data.target.off_speed_changed = true
        else
            addon.data.target.off_speed_changed = false
        end
    end
...
--[[===================================== VISUALS RELATED ======================================]]--
--[[============================================================================================]]--
addon.data.target.UpdateVisualsOnUpdate = function()
    local settings = character_target_settings
    local frame = addon.data.target.frame
    if settings.enabled and UnitExists("target") then
        frame:Show()
        local main_speed = addon.data.target.main_weapon_speed
        local main_timer = addon.data.target.main_swing_timer
        ...
        if addon.data.target.has_offhand and settings.show_offhand then
            frame.off_bar:Show()
            ...
            local off_speed = addon.data.target.off_weapon_speed
            local off_timer = addon.data.target.off_swing_timer
            -- FIXME: Handle divide by 0 error
            if off_speed == 0 then
                off_speed = 2
            end
        ...
        if addon.data.target.has_offhand and character_target_settings.show_offhand then
...
addon.data.target.UpdateVisualsOnSettingsChange = function()
    local frame = addon.data.target.frame
    local settings = character_target_settings
    if settings.enabled then
        ...
        if settings.show_offhand and addon.data.target.has_offhand then
            frame.off_bar:Show()
            if settings.show_left_text then
                frame.off_left_text:Show()
            else
...
addon.data.target.OnFrameDragStart = function()
    if not character_target_settings.is_locked then
        addon.data.target.frame:StartMoving()
    end
end

addon.data.target.OnFrameDragStop = function()
    local frame = addon.data.target.frame
    local settings = character_target_settings
...
addon.data.target.InitializeVisuals = function()
    local settings = character_target_settings
    -- Create the frame
    addon.data.target.frame = CreateFrame("Frame", addon.name .. "TargetFrame", UIParent)
    local frame = addon.data.target.frame
    frame:SetMovable(true)
    frame:EnableMouse(not settings.is_locked)
    frame:RegisterForDrag("LeftButton")
    frame:SetScript("OnDragStart", addon.data.target.OnFrameDragStart)
    frame:SetScript("OnDragStop", addon.data.target.OnFrameDragStop)
    -- Create the backplane
    frame.backplane = CreateFrame("Frame", addon.name .. "TargetBackdropFrame", frame, "BackdropTemplate")
...
addon.data.target.UpdateConfigPanelValues = function()
    local panel = addon.data.target.config_frame
    local settings = character_target_settings
...
addon.data.target.EnabledCheckBoxOnClick = function(self)
    character_target_settings.enabled = self:GetChecked()
    addon.data.target.UpdateVisualsOnSettingsChange()
end

addon.data.target.ShowOffHandCheckBoxOnClick = function(self)
    character_target_settings.show_offhand = self:GetChecked()
    addon.data.target.UpdateVisualsOnSettingsChange()
end

addon.data.target.ShowBorderCheckBoxOnClick = function(self)
    character_target_settings.show_border = self:GetChecked()
    addon.data.target.UpdateVisualsOnSettingsChange()
end

addon.data.target.ClassicBarsCheckBoxOnClick = function(self)
    character_target_settings.classic_bars = self:GetChecked()
    addon.data.target.UpdateVisualsOnSettingsChange()
end

addon.data.target.FillEmptyCheckBoxOnClick = function(self)
    character_target_settings.fill_empty = self:GetChecked()
    addon.data.target.UpdateVisualsOnSettingsChange()
end

addon.data.target.ShowLeftTextCheckBoxOnClick = function(self)
    character_target_settings.show_left_text = self:GetChecked()
    addon.data.target.UpdateVisualsOnSettingsChange()
end

addon.data.target.ShowRightTextCheckBoxOnClick = function(self)
    character_target_settings.show_right_text = self:GetChecked()
    addon.data.target.UpdateVisualsOnSettingsChange()
end

addon.data.target.WidthEditBoxOnEnter = function(self)
    character_target_settings.width = tonumber(self:GetText())
    addon.data.target.UpdateVisualsOnSettingsChange()
end

addon.data.target.HeightEditBoxOnEnter = function(self)
    character_target_settings.height = tonumber(self:GetText())
    addon.data.target.UpdateVisualsOnSettingsChange()
end

addon.data.target.FontSizeEditBoxOnEnter = function(self)
    character_target_settings.fontsize = tonumber(self:GetText())
    addon.data.target.UpdateVisualsOnSettingsChange()
end

addon.data.target.XOffsetEditBoxOnEnter = function(self)
    character_target_settings.x_offset = tonumber(self:GetText())
    addon.data.target.UpdateVisualsOnSettingsChange()
end

addon.data.target.YOffsetEditBoxOnEnter = function(self)
    character_target_settings.y_offset = tonumber(self:GetText())
    addon.data.target.UpdateVisualsOnSettingsChange()
end

addon.data.target.MainColorPickerOnClick = function()
    local settings = character_target_settings
    local function MainOnActionFunc(restore)
        ...
        addon.data.target.frame.main_bar:SetVertexColor(
            settings.main_r, settings.main_g, settings.main_b, settings.main_a)
        addon.data.target.config_frame.main_color_picker.foreground:SetColorTexture(
            settings.main_r, settings.main_g, settings.main_b, settings.main_a)
    end
...
addon.data.target.MainTextColorPickerOnClick = function()
    local settings = character_target_settings
    local function MainTextOnActionFunc(restore)
        ...
        addon.data.target.frame.main_left_text:SetTextColor(
            settings.main_text_r, settings.main_text_g, settings.main_text_b, settings.main_text_a)
        addon.data.target.frame.main_right_text:SetTextColor(
            settings.main_text_r, settings.main_text_g, settings.main_text_b, settings.main_text_a)
        addon.data.target.config_frame.main_text_color_picker.foreground:SetColorTexture(
            settings.main_text_r, settings.main_text_g, settings.main_text_b, settings.main_text_a)
    end
    ColorPickerFrame.func, ColorPickerFrame.opacityFunc, ColorPickerFrame.cancelFunc = 
...
addon.data.target.OffColorPickerOnClick = function()
    local settings = character_target_settings
    local function OffOnActionFunc(restore)
        ...
        addon.data.target.frame.off_bar:SetVertexColor(
            settings.off_r, settings.off_g, settings.off_b, settings.off_a)
        addon.data.target.config_frame.off_color_picker.foreground:SetColorTexture(
            settings.off_r, settings.off_g, settings.off_b, settings.off_a)
    end
    ColorPickerFrame.func, ColorPickerFrame.opacityFunc, ColorPickerFrame.cancelFunc = 
...
addon.data.target.OffTextColorPickerOnClick = function()
    local settings = character_target_settings
    local function OffTextOnActionFunc(restore)
        ...
        addon.data.target.frame.off_left_text:SetTextColor(
            settings.off_text_r, settings.off_text_g, settings.off_text_b, settings.off_text_a)
        addon.data.target.frame.off_right_text:SetTextColor(
            settings.off_text_r, settings.off_text_g, settings.off_text_b, settings.off_text_a)
        addon.data.target.config_frame.off_text_color_picker.foreground:SetColorTexture(
            settings.off_text_r, settings.off_text_g, settings.off_text_b, settings.off_text_a)
    end
    ColorPickerFrame.func, ColorPickerFrame.opacityFunc, ColorPickerFrame.cancelFunc = 
...
addon.data.target.CombatAlphaOnValChange = function(self)
    character_target_settings.in_combat_alpha = tonumber(self:GetValue())
    addon.data.target.UpdateVisualsOnSettingsChange()
end

addon.data.target.OOCAlphaOnValChange = function(self)
    character_target_settings.ooc_alpha = tonumber(self:GetValue())
    addon.data.target.UpdateVisualsOnSettingsChange()
end

addon.data.target.BackplaneAlphaOnValChange = function(self)
    character_target_settings.backplane_alpha = tonumber(self:GetValue())
    addon.data.target.UpdateVisualsOnSettingsChange()
end

addon.data.target.CreateConfigPanel = function(parent_panel)
    addon.data.target.config_frame = CreateFrame("Frame", addon.name .. "TargetConfigPanel", parent_panel)
    local panel = addon.data.target.config_frame
    local settings = character_target_settings
    -- Title Text
    panel.title_text = addon.data.config.TextFactory(panel, L["Target Swing Bar Settings"], 20)
    ...
    panel.enabled_checkbox = addon.data.config.CheckBoxFactory(
        ...
        addon.data.target.EnabledCheckBoxOnClick)
    ...
    panel.show_offhand_checkbox = addon.data.config.CheckBoxFactory(
        ...
        addon.data.target.ShowOffHandCheckBoxOnClick)
    ...
    panel.show_border_checkbox = addon.data.config.CheckBoxFactory(
        ...
        addon.data.target.ShowBorderCheckBoxOnClick)
    ...
    panel.classic_bars_checkbox = addon.data.config.CheckBoxFactory(
        ...
        addon.data.target.ClassicBarsCheckBoxOnClick)
    ...
    panel.fill_empty_checkbox = addon.data.config.CheckBoxFactory(
        ...
        addon.data.target.FillEmptyCheckBoxOnClick)
    ...
    panel.show_left_text_checkbox = addon.data.config.CheckBoxFactory(
        ...
        addon.data.target.ShowLeftTextCheckBoxOnClick)
    ...
    panel.show_right_text_checkbox = addon.data.config.CheckBoxFactory(
        ...
        addon.data.target.ShowRightTextCheckBoxOnClick)
    ...
    panel.width_editbox = addon.data.config.EditBoxFactory(
        ...
        75,
        25,
        addon.data.target.WidthEditBoxOnEnter)
    ...
    panel.height_editbox = addon.data.config.EditBoxFactory(
        ...
        75,
        25,
        addon.data.target.HeightEditBoxOnEnter)
	...
	panel.fontsize_editbox = addon.data.config.EditBoxFactory(
        ...
        75,
        25,
        addon.data.target.FontSizeEditBoxOnEnter)
    ...
    panel.x_offset_editbox = addon.data.config.EditBoxFactory(
        ...
        75,
        25,
        addon.data.target.XOffsetEditBoxOnEnter)
    ...
    panel.y_offset_editbox = addon.data.config.EditBoxFactory(
        ...
        75,
        25,
        addon.data.target.YOffsetEditBoxOnEnter)
    ...
    panel.main_color_picker = addon.data.config.color_picker_factory(
        ...
        addon.data.target.MainColorPickerOnClick)
    ...
    panel.main_text_color_picker = addon.data.config.color_picker_factory(
        ...
        addon.data.target.MainTextColorPickerOnClick)
    ...
    panel.off_color_picker = addon.data.config.color_picker_factory(
        ...
        addon.data.target.OffColorPickerOnClick)
    ...
    panel.off_text_color_picker = addon.data.config.color_picker_factory(
        ...
        addon.data.target.OffTextColorPickerOnClick)
    ...
    panel.in_combat_alpha_slider = addon.data.config.SliderFactory(
        ...
        1,
        0.05,
        addon.data.target.CombatAlphaOnValChange)
    ...
    panel.ooc_alpha_slider = addon.data.config.SliderFactory(
        ...
        1,
        0.05,
        addon.data.target.OOCAlphaOnValChange)
    ...
    panel.backplane_alpha_slider = addon.data.config.SliderFactory(
        ...
        1,
        0.05,
        addon.data.target.BackplaneAlphaOnValChange)
